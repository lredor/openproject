...
Formulas support the literals, operators, functions and keywords listed below, and can reference other project attributes — integer, float, boolean and weighted item list scores, and other calculated values.
...
When used as operators, `AND` and `OR` have the same lowest precedence `0`, lower than any other operators.
